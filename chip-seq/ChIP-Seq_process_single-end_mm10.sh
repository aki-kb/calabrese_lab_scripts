...
do
	...
	## 3) Filter and convert mm10-aligned reads by MAPQ>=30 in bam format
	jid2=$(sbatch --dependency=afterok:$jid1 -J ${file}.map30.bam --time=02:00:00 --wrap="samtools view -bhq 30 sam_files/${file}_mm10.sam > bam_files/${file}_mm10_q30.bam")
	...
	## 5) Sort bam file by coordinate
	jid4=$(sbatch --dependency=afterok:$jid2 -J ${file}.bam.sorting --mem=2G --time=04:00:00 --wrap "samtools sort -o bam_files/${file}_mm10_q30.sorted.bam bam_files/${file}_mm10_q30.bam")
	jid4=$(echo $jid4 | awk '{print $NF}') 

	## 6) Count the number of mm10 reads in genomic bins for tiling density analysis: 10-kb bins/no slide
	jid5=$(sbatch --dependency=afterok:$jid4 --mem=2G --time=04:00:00 -J ${file}.tiling -o reports/${file}.%j.tiling.out -e reports/${file}.%j.tiling.err --wrap="bedtools coverage -counts -sorted -g ${assets}/mm10.chr.sizes.sorted -a ${assets}/all-chr_mm10_10kb-bin_sorted.bed -b bam_files/${file}_mm10_q30.sorted.bam > binned_counts/${file}_mm10_all-chr_10kb-bins.txt")
	jid5=$(echo $jid5 | awk '{print $NF}')

	## 7) Gzip fastq
	jid6=$(sbatch --dependency=afterok:$jid1 --wrap="gzip fastq/${file}.fastq")
